...
(function() {
	...
	addReplacement('async loadSpritesheet(){', `
		async loadVape() {
			...
			this.v4Texture = await this.loader.loadAsync("https://raw.githubusercontent.com/BHThacker0809/mini/main/thrax.png");
		}
	...
	addReplacement('(this.drawSelectedItemStack(),this.drawHintBox())', `
		if (ctx$3 && enabledModules["TextGUI"]) {
			...
			drawImage(ctx$3, textureManager.vapeTexture.image, posX, posY + 25, 80, 21, \`HSL(\${(colorOffset % 1) * 360}, 100%, 50%)\`);
			drawImage(ctx$3, textureManager.v4Texture.image, posX + 81, posY + 26, 80, 21);
			...
			for(const module of stringList) {
				offset++;
				drawText(ctx$3, module, posX + 6, posY + 38 + ((textguisize[1] + 3) * offset), textguisize[1] + "px " + textguifont[1], \`HSL(\${((colorOffset - (0.025 * offset)) % 1) * 360}, 100%, 50%)\`, "left", "top", 1, textguishadow[1]);
			}
		}
	`);
	...
	addReplacement('async downloadSkin(_){', `
		if (_ == "GrandDad") {
			...
			return new Promise((et, tt) => {
				textureManager.loader.load("https://raw.githubusercontent.com/BHThacker0809/mini/main/skin.png", rt => {
					const nt = {
						atlas: rt,
						id: _,
						skinny: $.skinny,
	...
	addReplacement('async downloadCape(_){', `
		if (_ == "GrandDad") {
			const $ = capes[_];
			return new Promise((et, tt) => {
				textureManager.loader.load("https://raw.githubusercontent.com/BHThacker0809/mini/main/cape.png", rt => {
					const nt = {
						atlas: rt,
						id: _,
						name: $.name,
...
let intervalId2 = setInterval(() => {
    ...
    const imgElements = document.querySelectorAll('img[src="/assets/default-92b37f60.png"]');
    imgElements.forEach(element => {
        element.src = 'https://wallpaperaccess.com/full/1638902.jpg';
    });
...
function restoreMouse() {
    console.log("Restoring mouse visibility...");

    document.body.style.cursor = 'auto';
    console.log("Mouse cursor visibility restored.");
    ...
    setTimeout(() => {
        ...
        document.dispatchEvent(event);
        console.log("Dispatched Escape key event after 1 second.");
    }, 1000);
}
...
title.innerText = 'Thrax Client Control Panel V1.0 | VAPE V4.0';
...
function showShortcutPopup(buttonText, button) {
    ...
    activePopup.style.color = '#ABFFFFFF';
    ...
    instruction.innerText = 'Set a shortcut key for ' + buttonText + ' (press ESC to cancel)';
    instruction.style.margin = '0';
    activePopup.appendChild(instruction);
    ...
    function handleKeydown(event) {
        if (event.key === 'Escape') {
            ...
            if (buttonShortcuts[buttonText]) {
                showToast('Warning: ' + buttonText + ' has been bound to key ' + buttonShortcuts[buttonText]);
            }
            buttonShortcuts[buttonText] = event.key;
            showToast(buttonText + ' is now bound to ' + event.key);
            removePopup();
        }
    }
...
function handleGlobalKeydown(event) {
    ...
    if (shortcutKey) {
        ...
        const buttonAction = getModule(shortcutKey);

        if (buttonAction) {

            const button = Array.from(document.querySelectorAll("button")).find(btn => btn.innerText === shortcutKey);
            if (button) {
...
let intervalId = null;
const longIntervalTime = 1000000000000000;
const shortIntervalTime = 1000;

new Module("CreativeInventory", function(callback) {
    if (callback) {
        function replaceClassOn() {
            const elements = document.querySelectorAll('.css-13x5l11');
            elements.forEach(function (element) {
                element.classList.remove('css-13x5l11');
                element.classList.add('css-eh8ku1');
            });
        }

        clearInterval(intervalId);

        intervalId = setInterval(replaceClassOn, shortIntervalTime);
    } else {

        clearInterval(intervalId);

        intervalId = setInterval(() => {}, longIntervalTime);

        function replaceClassOff() {
            const elements = document.querySelectorAll('.css-eh8ku1');
            elements.forEach(function (element, index) {
                if (index > 0) {
                    element.classList.remove('css-eh8ku1');
                    element.classList.add('css-13x5l11');
                }
            });
        }
        ...
        replaceClassOff();
			...
			chatdisablermsg = chatdisabler.addoption("Message", String, "https://www.youtube.com/@BHT_hacker + https://youtube.com/c/7GrandDadVape");
